...
- **A declared envelope's frame origin sat a half-height below a detected one** (#977). It
  used `bbox_min.Z` where the detector uses the bbox centre. Drawing output is unchanged —
  sizes derive from `bbox_min`/`bbox_max`, and the overall height is compiled from the model
  bbox — but `frame.origin` is the generic feature *site*, so **a GD&T control frame, surface
  finish or note targeting a hand-declared `sheet.envelope()` was anchored at the bottom face
  rather than the centre**. It now matches detection. Scripts generated by `--script` were
  never affected: they bake the detected frame explicitly.
