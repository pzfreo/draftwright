...
**2026-07-18 (#636 close-out):** `render_height_ladder` migrated to corridor candidates — the leapfrog witness cursor survives as a build-time chain over the solved-position map, and the overall height rides the OVERALL subchain (outermost by construction). The `render_gdt` alt-strip fallback now defers via `ctx.post_drain` like the plate fallthrough. Remaining `carve_free_position` callers are each explicit exemptions: the two post-drain fallthroughs (gdt, plates), the manual post-build verbs `add_feature_callout`/`add_feature_location` (a single annotation onto a finished sheet — the #426 manual-edit half; no drain to join), and `_place_pitch_dim` (permanent, Amendment 8: a diagonal fallback vector cannot occupy a 1-D tier). The by-construction guarantee now covers every auto-pass strip occupant. Known residual (dshape): a witness crossing a fixed-height label is a perpendicular-axis conflict tier co-solving cannot fix — remedy is label-offset shifting for witness crossings (follow-up issue).
