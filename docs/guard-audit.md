...
| `test_consumer_tests_use_only_released_recogniser_contract_or_explicit_blockers` | **keep** | #1411: all consumer suites use released provider contracts except the exact shrinking allowlist blocked by upstream #400/#408. This is intentionally a provider-specific static boundary, not a Python interpreter: it checks imports, straightforward package aliases/attributes, literal `getattr`, direct provider-object member calls, and literal dotted provider targets in assignments/calls. General reflection and dynamically constructed names remain review concerns. Dynamic public-root iteration is centralized behind immutable captured authorization and a read-only live root view, so instrumentation can replace a published value but cannot authorize a private name. Provider-owned predicate coverage lives in released v0.4.10; the versioned public `inspection` namespace is explicitly distinguished from implementation modules. |
