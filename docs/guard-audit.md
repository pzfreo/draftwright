...
| `recognition_consumer_calls` (`conftest.py`) | **keep** | ADR 3 (was 0017) consumer ownership: counts the public aggregate and catches public physical-recogniser bypasses without inspecting provider internals. Provider registry completeness is owned by the released provider suite. |
| `test_consumer_tests_use_only_released_recogniser_contract_or_explicit_blockers` | **keep** | #1411: all consumer suites use released provider contracts except the exact shrinking allowlist blocked by upstream #400/#408. This is intentionally a provider-specific static boundary, not a Python interpreter: it checks imports, straightforward package aliases/attributes, literal `getattr`, direct provider-object member calls, and literal dotted provider targets in assignments/calls. General reflection and dynamically constructed names remain review concerns. Dynamic public-root iteration is centralized behind immutable captured authorization and a read-only live root view, so instrumentation can replace a published value but cannot authorize a private name. Provider-owned predicate coverage lives in released v0.4.14; the versioned public `inspection` namespace is explicitly distinguished from implementation modules. |
...
| `test_declared_recognition_gate.py` | **keep** | ADR 3 (was 0017): declared builds must not pay recognition cost; regression is silent (slow, not wrong). |
...
| `test_architecture_docs.py` | **trimmed** | Three of four assertions policed phrase absences from battles won weeks ago (stale ADR 1 (was 0008) / ADR 2 (was 0009) references); only the live no-line-anchors rule survives. |
...
counter, emit a named diagnostic when it fires (per ADR 2 (was 0014 Amdt 4)), and have a
