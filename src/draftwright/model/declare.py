...
def double_d_bore(
    ...
    *,
    ...
    """
    if obj is not None:
        from quiddity.inspection import read_double_d_tool
...
def read_countersink(cone) -> tuple[float, float]:
    ...
    :func:`~quiddity.inspection.cone_rims` (#704), so a declared
    countersink reads identically to a detected one by construction."""
    from build123d import GeomType
    from quiddity.inspection import cone_rims
...
def _read_chamfer_face(face) -> tuple[str, float, float, Point]:
    ...
    recogniser's own :func:`~quiddity.inspection.classify_bevel` (#704), so a
    declared chamfer reads identically to a detected one by construction; only the
    user-facing error messages live here."""
    from quiddity.inspection import BevelReject, classify_bevel
...
def _read_groove_face(face) -> tuple[str, float, float, Point]:
    ...
    from quiddity.inspection import floor_face_anchor
