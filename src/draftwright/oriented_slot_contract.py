...
def oriented_slot_provider_key(slot) -> tuple:
    """Validate one exact released record and return its lossless geometric correspondence key."""
    ...
    for vertex in boundary_source:
        ...
        _serialized(point, 4, name="oriented slot boundary point")
...
def standalone_oriented_slots(slots: tuple, patterns: tuple) -> tuple[OrientedSlot, ...]:
    """Select standalone records by exact same-run pattern membership, never geometry."""
    if type(slots) is not tuple or type(patterns) is not tuple:
        raise TypeError("oriented slot inventories must be exact immutable tuples")
    if len({id(slot) for slot in slots}) != len(slots):
        raise ValueError("oriented slot inventory repeats one record")
    remaining = list(slots)
    for slot in slots:
        oriented_slot_provider_key(slot)
    for pattern in patterns:
        for member, _key in validate_oriented_slot_pattern(pattern):
            matches = [index for index, candidate in enumerate(remaining) if candidate is member]
            if len(matches) != 1:
                raise ValueError(
                    "oriented slot pattern member is not an exact unclaimed inventory record"
                )
