...
def test_real_fillet_corpus_scores_all_layers_and_topology_variants() -> None:
    ...
    assert evaluation.detection.matched == 14
    assert evaluation.parameter_fidelity.passed == evaluation.parameter_fidelity.total == 14
    assert evaluation.downstream_usefulness.passed == evaluation.downstream_usefulness.total == 56
...
def test_every_fillet_boundary_is_observed_supported_on_the_real_public_path() -> None:
    ...
    observed = _default_observers()["fillets"](_lone())
    assert len(observed) == 1
