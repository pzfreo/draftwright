...
def test_infeasible_scale_selection_records_the_warning(scale, page, caplog):
    ...
    assert not [
        ...
        if record.levelno >= logging.WARNING and "may not fit" in record.getMessage()
    ]
...
def test_candidate_repack_warning_waits_for_the_delivered_sheet(monkeypatch, caplog, tmp_path):
    import draftwright.builder as builder
    ...
    assert not [
        record
        for record in caplog.records
        if record.levelno >= logging.WARNING and "measure-repack" in record.getMessage()
    ]

    caplog.clear()
    drawing.export(str(tmp_path / "unresolved"), formats=("svg",))
    assert any(
        record.levelno >= logging.WARNING and "layout_repack_stalled" in record.getMessage()
        for record in caplog.records
    )
