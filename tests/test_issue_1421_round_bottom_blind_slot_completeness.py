"""#1421 — round-bottom blind-slot completeness uses independent physical facts."""
...
def _part(*, flat_width=4, radius=3, length=20):
...
_CASES = (
    _Case("z-positive", _part(), "z", 1, "x", "y", 1, 20, 3, 4, (0, -1.5, 10)),
    _Case(
        "z-sized",
        _part(flat_width=5, radius=2, length=15),
        "z",
        1,
        "x",
        "y",
...
def _source(case: _Case):
    ...
    return recognition, recognition.round_bottom_blind_slots[0]
...
def test_independent_corpus_reaches_recognition_ir_and_finished_measurements(case) -> None:
    ...
    outcomes = round_bottom_blind_slot_requirement_outcomes(
    ...
    assert not [
        ...
        if issue.code.startswith("round_bottom_blind_slot_requirement_")
    ]
    completeness = drawing.lint_summary()["quality"]["completeness"]
    assert completeness["by_family"]["round_bottom_blind_slots"] == 3
    assert "round_bottom_blind_slots" not in completeness["unscored_recognized_families"]
...
def test_corpus_denominator_does_not_shrink_when_recognition_is_removed() -> None:
    ...
    for case in _CASES:
        ...
        observed += 3 * len(recognition.round_bottom_blind_slots)
        weakened = replace(recognition, round_bottom_blind_slots=())
        damaged += len(
            round_bottom_blind_slot_requirement_outcomes(weakened, (), AnnotationRegistry())
        )
...
def test_declared_and_executed_generated_sheet_paths_earn_the_same_outcomes() -> None:
    ...
    declared_handle = declared.round_bottom_blind_slot(**case.declaration())
...
def test_authored_subset_is_suppressed_not_silently_complete() -> None:
    ...
    handle = sheet.round_bottom_blind_slot(**case.declaration())
...
def test_ledger_distinguishes_every_engine_outcome_and_duplicate_sources() -> None:
    ...
    dropped.record_issue(
        LintIssue(
            ...
            code="round_bottom_blind_slot_dropped",
    ...
    duplicated = replace(
        ...
        round_bottom_blind_slots=(
            recognition.round_bottom_blind_slots[0],
            recognition.round_bottom_blind_slots[0],
        ),
    )
    ambiguous = round_bottom_blind_slot_requirement_outcomes(
...
def test_correspondence_rejects_near_neighbours_and_malformed_ir_parameters() -> None:
    ...
    wrong_size = replace(feature, flat_width=feature.flat_width + 1)
    assert {
        outcome.state
        for outcome in round_bottom_blind_slot_requirement_outcomes(
    ...
    malformed_parameters = (
        (
            replace(parameters[0], role="round_bottom_blind_slot_unknown"),
            *parameters[1:],
        ),
    ...
    for malformed in malformed_parameters:
        ...
        assert round_bottom_blind_slot_key(
            proxy, require_frame=True
        ) == round_bottom_blind_slot_key(source)
        assert {
            outcome.state
            for outcome in round_bottom_blind_slot_requirement_outcomes(
...
def test_outcome_boundary_rejects_a_foreign_aggregate() -> None:
    ...
    mutable = replace(
        recognition,
        round_bottom_blind_slots=list(recognition.round_bottom_blind_slots),  # type: ignore[arg-type]
    )
    with pytest.raises(TypeError, match="immutable tuple"):
        round_bottom_blind_slot_requirement_outcomes(mutable, (), AnnotationRegistry())
...
def test_correspondence_key_rejects_every_malformed_released_fact() -> None:
    ...
    for malformed in malformed_sources:
        with pytest.raises(ValueError):
            round_bottom_blind_slot_key(malformed)

    malformed_recognition = replace(_recognition, round_bottom_blind_slots=(malformed_sources[4],))
    malformed_outcomes = round_bottom_blind_slot_requirement_outcomes(
    ...
    mismatched_frame = SimpleNamespace(
        ...
        flat_width=feature.flat_width,
        frame=Frame(feature.frame.origin, feature.width_axis),
    )
    with pytest.raises(ValueError):
        round_bottom_blind_slot_key(mismatched_frame, require_frame=True)
