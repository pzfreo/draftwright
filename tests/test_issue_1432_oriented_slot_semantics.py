...
def test_public_aggregate_lowers_to_a_dedicated_lossless_ir() -> None:
    ...
    assert feature.length == pytest.approx(24.0, abs=0.002)
    assert feature.width_direction == pytest.approx((-0.5, math.sqrt(3) / 2, 0), abs=3e-5)
    assert feature.long_direction == pytest.approx((math.sqrt(3) / 2, 0.5, 0), abs=3e-5)
...
def test_authored_roles_tolerance_and_executed_generated_sheet_round_trip() -> None:
    ...
    expected = {"ORIENTED SLOT 6 ±0.1 WIDE × 24 LONG"}
    assert {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("oriented_slot_length.length", "ORIENTED SLOT 24 LONG"),
    ],
)
...
def test_drawing_places_one_solver_owned_two_requirement_callout() -> None:
    ...
    assert annotation.label == "ORIENTED SLOT 6 WIDE × 24 LONG"
...
@pytest.mark.parametrize(
    ("rotation", "expected_view", "camera_axis", "camera_sign"),
    [((17, 23, 31), "plan", 2, 1), ((70, 23, 31), "front", 1, -1), ((17, 70, 31), "side", 0, 1)],
)
@pytest.mark.parametrize(
    "parameter_id",
    [None, "oriented_slot_width.length", "oriented_slot_length.length"],
...
def test_raw_moved_leader_targets_the_camera_facing_physical_opening(
    rotation, expected_view, camera_axis, camera_sign, parameter_id
) -> None:
    moved = Pos(13, -27, 41) * Rot(*rotation) * _part()
    if parameter_id is None:
        drawing = build_drawing(moved)
    else:
        sheet = Sheet(moved)
        handle = sheet.oriented_slot(**_kwargs(_feature(moved)))
        sheet.authored_dimensions().dimension(handle, parameter_id)
        drawing = sheet.build()
    feature = next(item for item in drawing.model().features if item.kind == "oriented_slot")
    name, annotation = next(iter(drawing.annotations_of(feature).items()))
    assert drawing.view_of(name) == expected_view

    # The actual solid supplies eight 6/24 mm opening edges, on two distinct faces.
    # Choose the camera-facing face independently of provider stations and IR geometry.
    mouths = [face for face in moved.faces() if len(face.inner_wires()) == 1]
    assert len(mouths) == 2
    facing = max(mouths, key=lambda face: camera_sign * tuple(face.center())[camera_axis])
    edges = list(facing.inner_wires()[0].edges())
    assert sorted(edge.length for edge in edges) == pytest.approx([6, 6, 24, 24])
    segments = [
        tuple(drawing.at(expected_view, *tuple(vertex))[:2] for vertex in edge.vertices())
        for edge in edges
    ]
    assert all(len(segment) == 2 for segment in segments)
    distance = min(
        _point_segment_distance(annotation.tip[:2], start, end) for start, end in segments
    )
    assert distance < 0.01
...
def test_pattern_ownership_consumes_each_equal_source_occurrence_only_once() -> None:
    ...
    extra = copy(sources[0])
    assert extra == sources[0] and extra is not sources[0]
    duplicated = replace(
        recognition,
        oriented_slots=(extra, *sources),
        oriented_slot_patterns=patterns,
    )
    from draftwright.oriented_slot_contract import standalone_oriented_slots

    assert standalone_oriented_slots(duplicated.oriented_slots, patterns)[0] is extra
