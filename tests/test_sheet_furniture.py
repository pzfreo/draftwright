...
class TestSheetFrame:
    ...
    def test_frame_off_by_default(self, shared_drawing):
        dwg = shared_drawing("box_80x60x20")
        assert "sheet_frame" not in dwg.annotations()
        assert dwg._analysis.margin == _MARGIN  # no reservation

    def test_frame_drawn_at_the_margin_and_content_clears_it(self, shared_drawing):
        dwg = shared_drawing("box_80x60x20", frame=True)
        a = dwg._analysis
    ...
    def test_frame_build_is_lint_clean(self, shared_drawing):
        dwg = shared_drawing("box_80x60x20", frame=True)
...
class TestProjectionSymbol:
    ...
    def test_third_angle_symbol_by_default(self, shared_drawing):
        dwg = shared_drawing("box_80x60x20")
        assert dwg.get_annotation("projection_symbol").method == "third"
        assert dwg._analysis.projection is None

    def test_third_renders_in_the_title_block_band(self, shared_drawing):
        from draftwright._core import _TB_CLEAR, _TB_H

        dwg = shared_drawing("box_80x60x20", projection="third")
    ...
    def test_projection_build_is_lint_clean(self, shared_drawing):
        dwg = shared_drawing("box_80x60x20", projection="third")
        by_code = dwg.lint_summary()["by_code"]
        assert by_code.get("annotation_overlap", 0) == 0
        assert by_code.get("annotation_out_of_bounds", 0) == 0
...
class TestZoneGrid:
    ...
    def test_off_by_default(self, shared_drawing):
        dwg = shared_drawing("box_80x60x20")
    ...
    def test_labels_sit_in_the_border_band(self, shared_drawing):
        dwg = shared_drawing("box_80x60x20", zones=True)
    ...
    def test_zone_build_is_lint_clean(self, shared_drawing):
        dwg = shared_drawing("box_80x60x20", zones=True)
        by_code = dwg.lint_summary()["by_code"]
        assert by_code.get("annotation_overlap", 0) == 0
        assert by_code.get("annotation_out_of_bounds", 0) == 0
